...
uniform vec3 u_5d42e63c_0;
uniform float u_5d42e63c_1;
uniform mat4 u_5d42e63c_2;
uniform vec3 u_da8df2e7_0;
uniform float u_da8df2e7_1;
uniform mat4 u_da8df2e7_2;
...
float sdf_eval_0(vec3 p) {
    return sdf_sphere(apply_inv_mat(u_5d42e63c_2, p), u_5d42e63c_0, u_5d42e63c_1);
}
...
float sdf_eval_1(vec3 p) {
    return sdf_sphere(apply_inv_mat(u_da8df2e7_2, p), u_da8df2e7_0, u_da8df2e7_1);
}

vec3 sdf_normal_1(vec3 p) {
    vec2 e = vec2(0.001, 0.0);
    vec3 n = sdf_eval_1(p) - vec3(
        sdf_eval_1(p - e.xyy),
        sdf_eval_1(p - e.yxy),
        sdf_eval_1(p - e.yyx)
    );
...
void main() {
    ...
    vec3 bmin_1 = vec3(5.0, -5.0, -5.0);
    vec3 bmax_1 = vec3(15.0, 5.0, 5.0);


    vec3 scene_min = bmin_0;
    vec3 scene_max = bmax_0;
    scene_min = min(scene_min, bmin_1);
    scene_max = max(scene_max, bmax_1);
    ...
    float ftn[2]; float ftf[2];
    vec2 fi_int_0 = intersect_aabb(ro, rd, bmin_0, bmax_0);
    ftn[0] = max(fi_int_0.x, 0.0);
    ftf[0] = min(fi_int_0.y, ray_tmax);
    vec2 fi_int_1 = intersect_aabb(ro, rd, bmin_1, bmax_1);
    ftn[1] = max(fi_int_1.x, 0.0);
    ftf[1] = min(fi_int_1.y, ray_tmax);
    ...
    for (int i = 0; i < 512; i++) {
        ...
        if (t >= ftn[1] && t <= ftf[1]) {
            float d_1 = sdf_eval_1(p);
            if (abs(d_1) < 0.005) { hit = true; hit_field = 1; break; }
            min_d = min(min_d, abs(d_1));
        } else if (t < ftn[1]) {
            min_d = min(min_d, ftn[1] - t);
        }
    ...
    if (hit_field == 1) n = sdf_normal_1(hp);
    ...
    if (hit_field == 1) base_color = vec3(1.0, 0.5, 0.0);
