    ...
    def undraw(self):
        ...
        if self._parent and self._sep:
            try:
                ...
                fld_logger.debug(f"FldLineSet remove child failed: {e}")
        self._sep = None
        self._coords = None
        self._lines = None
        self._parent = None
...
class FldPlaneVisual:
    """Manages a Coin3D semi-transparent plane for visual feedback of constraints."""
    def __init__(self, parent_node, color=(1.0, 1.0, 1.0), transparency=0.7):
        self._parent = parent_node
        self._sep = None
        self._coords = None
        self._face = None
        ...
        if coin and parent_node:
            ...
            # Disable backface culling for two-sided display
            ...
            self._sep.addChild(hints)
            
            self._coords = coin.SoCoordinate3()
            self._sep.addChild(self._coords)
            
            self._face = coin.SoFaceSet()
            self._sep.addChild(self._face)
            
            self._parent.addChild(self._sep)

    def update_plane(self, center, normal, size=150.0):
        if not self._coords or not self._face:
            return
            
        # Generate 4 points representing a square plane centered at 'center' perpendicular to 'normal'
        normal = FreeCAD.Vector(normal).normalize()
        if abs(normal.z) < 0.9:
            u = normal.cross(FreeCAD.Vector(0, 0, 1)).normalize()
        else:
            u = normal.cross(FreeCAD.Vector(1, 0, 0)).normalize()
        v = normal.cross(u).normalize()
        
        half = size / 2.0
        p1 = center - u * half - v * half
        p2 = center + u * half - v * half
        p3 = center + u * half + v * half
        p4 = center - u * half + v * half
        
        pts = [(p1.x, p1.y, p1.z), (p2.x, p2.y, p2.z), (p3.x, p3.y, p3.z), (p4.x, p4.y, p4.z)]
        self._coords.point.setValues(pts)
        self._face.numVertices.setValue(4)
    ...
    def undraw(self):
        if self._parent and self._sep:
            try:
                ...
                fld_logger.debug(f"FldPlaneVisual remove child failed: {e}")
        self._sep = None
        self._coords = None
        self._face = None
        self._parent = None
