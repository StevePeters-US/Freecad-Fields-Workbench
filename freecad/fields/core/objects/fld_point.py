...
class FldPoint:
    ...
    def __init__(self, position, handle_in=None, handle_out=None, weight=1.0):
        if isinstance(position, FldPoint):
    ...
    def draw_point(self, parent_node, radius=5.0, color=DEFAULT_ADDITIVE_COLOR):
        """
        ...
        if not coin or not hasattr(coin, "SoLightModel") or parent_node is None:
            return
    ...
    def undraw(self):
        ...
        try:
            ...
            fld_logger.debug(f"FldPoint undraw failed: {e}")
        self._point_sep = None
        self._point_mat = None
        self._point_xf = None
        self._point_sphere = None
    ...
    def __repr__(self):
        return f"FldPoint({self.position.x:.2f}, {self.position.y:.2f}, {self.position.z:.2f})"
