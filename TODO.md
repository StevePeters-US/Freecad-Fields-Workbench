...
### BUG-A. Suppress floating context menu on 3rd click (placement) (Complexity: 3/10)

- **Goal**: When the user places a primitive with the 3rd mouse click, a FreeCAD floating context menu pops up. It must not appear.
- **Background**: The 3rd click fires `handle_click` → `self.finish()` → `QtCore.QTimer.singleShot(0, self._do_finish)`. The mouse-button *release* that follows the same click is processed by FreeCAD's native 3D-view handler, which treats it as a context-menu trigger. The event callback must consume the click (return `True`) so FreeCAD does not pass it further.
- **Files to read**:
  - `FCDirectModeling/primitives/box_creator.py` lines 178–196 — the `event_cb` override. Note it returns `False` unconditionally after calling `handle_click`. That allows FreeCAD to continue processing the event and show its default context menu.
  - `FCDirectModeling/primitives/base.py` lines 146–169 — `PrimitiveCreatorBase.event_cb`. Also returns `False` unconditionally.
- **Files to modify**:
  - `FCDirectModeling/primitives/box_creator.py`
- **Steps**:
  1. In `BoxCreator.event_cb`, change the mouse-button handler so it **returns `True`** when `state == 2` (3rd click) and the left button is pressed. This tells Coin3D the event is consumed and prevents the context menu.
     ```python
     if event_type == "SoMouseButtonEvent":
         if event_dict["State"] == "DOWN":
             ...
             if button == "BUTTON1":
                 ...
                 # Consume the 3rd-click so FreeCAD doesn't show a context menu
                 if self.state == 2:   # state is already advanced inside handle_click
                     return True
     ```
     > **Note**: `handle_click` advances `self.state` before returning, so after the 3rd click `self.state` will be whatever `finish()` leaves it at (still 2, since finish defers via QTimer). Check which value `self.state` holds immediately after the call. Adjust the condition if needed (e.g. check state *before* calling `handle_click`).
  2. Re-test with 1st and 2nd clicks to confirm they still work normally (they should, since only the 3rd-click branch is consumed).
- **Acceptance**: Draw a box (click 1 → drag → click 2 → drag → click 3). No floating menu appears. The box is placed correctly.

---

### BUG-B. Final placed mesh has no geometry (Complexity: 4/10)

- **Goal**: After the 3rd click places a primitive, the object appears in the model tree but has no visible mesh in the viewport (the mesh is empty).
- **Background**: `_do_finish` in `base.py` calls `create_sdf_object`, which creates a `Mesh::FeaturePython` and triggers `doc.recompute()`. This calls `SDFObjectProxy.execute()`, which should mesh the SDF and write it to `fp.Mesh`. The mesh is empty, which means either (a) `execute()` returned early, (b) the params passed to `create_sdf_object` are wrong/degenerate, or (c) a second `doc.recompute()` (triggered by `terminate()` removing the preview object) replaces/clears the mesh.
- **Files to read**:
  - `FCDirectModeling/primitives/base.py`:
    - `_do_finish` (lines 307–333) — reads `self._last_sdf_type` and `self._last_sdf_params`.
    - `update_sdf_preview` (lines 206–227) — sets `self._last_sdf_type` and `self._last_sdf_params`.
    - `terminate` (lines 335–346) — removes preview object and calls `doc.recompute()` **after** the final object is created.
  - `FCDirectModeling/primitives/box_creator.py`:
    - `update_preview` (lines 150–176) — overrides `self._last_sdf_params` with the "real" (unclamped) bounds after calling `update_sdf_preview`. This is the dict that reaches `create_sdf_object`.
  - `FCDirectModeling/sdf_object.py`:
    - `create_sdf_object` (lines 295–322) — creates the object, calls `obj.touch()` then `doc.recompute()`.
    - `SDFObjectProxy.execute` (lines 215–282) — meshes the SDF. Has an early-return guard `if getattr(self, "is_preview", False)` and another if `len(verts) == 0`.
- **Diagnosis steps** (do these first, before changing any code):
  1. Add a temporary `FreeCAD.Console.PrintMessage` at the top of `SDFObjectProxy.execute` to print `fp.SDFType`, `fp.SDFParams`, and `fp.SDFOp`. Run the box creation and check the FreeCAD Report View. Confirm whether `execute` is being called at all, and what params it receives.
  2. Add a `PrintMessage` after `verts, tris = mesh_sdf(...)` to print `len(verts)` and `len(tris)`. If 0, the SDF bounds or function are wrong.
  3. Add a `PrintMessage` in `_do_finish` to print `self._last_sdf_type` and `self._last_sdf_params` right before calling `create_sdf_object`. Confirm the params have non-zero extents.
  4. Check whether `terminate()` → `doc.recompute()` is triggering a second `execute()` on the final object and whether anything clears `fp.Mesh` between the two calls.
- **Likely fixes** (implement whichever diagnosis reveals):
  - **If params are degenerate** (e.g. `bounds_min == bounds_max`): the height could be `0.0` at finalization time if the user finishes too quickly. Add a minimum thickness guard in `_do_finish` before calling `create_sdf_object`.
  - **If `execute` is not called**: `obj.touch()` + `doc.recompute()` sequence in `create_sdf_object` may be racing with something. Try calling `doc.recompute([obj])` (targeted recompute) instead.
  - **If the second recompute clears the mesh**: Defer `terminate()` so it runs *after* the first recompute fully completes, by wrapping it in `QtCore.QTimer.singleShot(50, self.terminate)` inside `_do_finish`.
- **Files to modify** (after diagnosis):
  - `FCDirectModeling/primitives/base.py` — `_do_finish` and/or `terminate`.
  - Possibly `FCDirectModeling/sdf_object.py` — `create_sdf_object`.
- **Acceptance**: Draw a box. On the 3rd click, the placed object immediately shows a solid mesh in the viewport identical (or higher-resolution) to the preview mesh. The FreeCAD Report View shows no errors.
